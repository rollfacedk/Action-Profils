local function MyRoutine()
	...
	local function Init()
		message( 'DEBUGING TEST ONLY. DO NOT USE', 1)
	...
	local function EnvelopingMistFunction(UnitTarget)
		return UnitTarget:BuffDown(S.EnvelopingMist)
	end;
	...
	local function MainRotation()
		...
		if S.EnvelopingMist:IsCastable() and Player:IsChanneling(S.SoothingMist) then
			if CastCycleAlly(S.EnvelopingMist, MEMBERS, EnvelopingMistFunction) then return end
		end
