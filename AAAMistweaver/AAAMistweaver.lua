local function MyRoutine()
	...
	local Author = '11.2 Fist Weaver - Mist Weaver'
	...
	local function SecretInfusionDown()
		return 
		Player:BuffDown(S.SecretInfusionCritBuff) and 
		Player:BuffDown(S.SecretInfusionHasteBuff) and 
		Player:BuffDown(S.SecretInfusionMasteryBuff) and 
		Player:BuffDown(S.SecretInfusionVersBuff)
	end
	...
	local function EnvelopingMistFunction(UnitTarget)
		return UnitTarget:HealthPercentage() <= 65 and UnitTarget:BuffDown(S.EnvelopingMist) and UnitTarget:BuffDown(S.LifeCocoon)
	end;

	local function EnvelopingMistFunction2(UnitTarget)
		return UnitTarget:HealthPercentage() <= 75 and UnitTarget:BuffDown(S.EnvelopingMist) and UnitTarget:BuffDown(S.LifeCocoon)
	end;
	...
	local function MainRotation()
		...
		if S.ExpelHarm:IsCastable() and Player:BuffDown(S.ThunderFocusTea) and Player:HealthPercentageFlat() <= 35 then
		...
		if S.Revival:IsCastable() and (HealingEngine:MembersUnderPercentage(65, nil, 40) >= 4) then
		...
		if (HealingEngine:MembersUnderPercentage(75, nil, 40) >= 4) then
			...
			if S.InvokeChiJiTheRedCrane:IsCastable() and Target:IsInMeleeRange(5) then
				if Cast(S.InvokeChiJiTheRedCrane) then return end
			end

			if S.CelestialConduit:IsCastable() then
		...
		if (HealingEngine:MembersUnderPercentage(85, nil, 40) >= 3) then
			...
			if S.SheilunsGift:IsCastable() and not Player:IsMoving() and S.SheilunsGift:Count() >= 8 then
		...
		if S.Vivify:IsCastable() and Player:BuffUp(S.VivaciousVivificationBuff) then
			if MainAddon.CastCycleAlly(S.Vivify, MEMBERS, VivifyFunction) then return end
		end
		...
		if MainAddon.TargetIsValid() then
			...
			-- chi_burst,if=active_enemies>=2
			if S.ChiBurst:IsReady() and Target:IsInMeleeRange(8) and EnemiesCount5 >= 2 then
				if Cast(S.ChiBurst) then return end
			end

			-- crackling_jade_lightning,if=buff.jade_empowerment.up
			if S.ThunderFocusTea:CooldownRemains() <= 12 then
