...
local InterruptEnabled = Action.GetToggle(4, "TargetMouseoverList")
local useKick, useCC, useRacial = Action.InterruptIsValid(unit, "TargetMouseover")
local InterruptWithStun = useCC
			
-- Interrupt
function Commons.Interrupt (Range, Spell, Setting, StunSpells)
    if InterruptEnabled and Target:IsInterruptible() and Target:IsInRange(Range) then
        if Spell:IsCastable() then
            if HR.Cast(Spell, Setting) then return "Cast " .. Spell:Name() .. " (Interrupt)"; end
        elseif InterruptWithStun and Target:CanBeStunned() then
            if StunSpells then
