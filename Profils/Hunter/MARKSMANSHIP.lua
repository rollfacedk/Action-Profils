...
local function APL() 
    ...
    local function Precombat()
        ...
        if Everyone.TargetIsValid() then
            ...
            if S.DoubleTap:IsCastableP() and not ShouldStop then
            ...
            if S.Trueshot:IsCastableP() and not ShouldStop and Player:BuffDownP(S.TrueshotBuff) and (EnemiesCount > 2) then
    ...
    local function Cds()
        ...
        if S.HuntersMark:IsCastableP() and not ShouldStop and (Target:DebuffDown(S.HuntersMarkDebuff) and Player:BuffDownP(S.TrueshotBuff)) then
        ...
        if S.DoubleTap:IsCastableP() and not ShouldStop and (S.RapidFire:CooldownRemainsP() < Player:GCD() or S.RapidFire:CooldownRemainsP() < S.AimedShot:CooldownRemainsP() or Target:TimeToDie() < 20) then
        ...
        if S.Berserking:IsCastableP() and not ShouldStop and HR.CDsON() and (Player:BuffP(S.TrueshotBuff) and (Target:TimeToDie() > S.Berserking:CooldownRemainsP() + S.Berserking:BaseDuration() or (Target:HealthPercentage() < 20 or not S.CarefulAim:IsAvailable())) or Target:TimeToDie() < 13) then
        ...
        if S.BloodFury:IsCastableP() and not ShouldStop and HR.CDsON() and (Player:BuffP(S.TrueshotBuff) and (Target:TimeToDie() > S.BloodFury:CooldownRemainsP() + S.BloodFury:BaseDuration() or (Target:HealthPercentage() < 20 or not S.CarefulAim:IsAvailable())) or Target:TimeToDie() < 16) then
        ...
        if S.AncestralCall:IsCastableP() and not ShouldStop and HR.CDsON() and (Player:BuffP(S.TrueshotBuff) and (Target:TimeToDie() > S.AncestralCall:CooldownRemainsP() + S.AncestralCall:BaseDuration() or (Target:HealthPercentage() < 20 or not S.CarefulAim:IsAvailable())) or Target:TimeToDie() < 16) then
        ...
        if S.Fireblood:IsCastableP() and not ShouldStop and HR.CDsON() and (Player:BuffP(S.TrueshotBuff) and (Target:TimeToDie() > S.Fireblood:CooldownRemainsP() + S.Fireblood:BaseDuration() or (Target:HealthPercentage() < 20 or not S.CarefulAim:IsAvailable())) or Target:TimeToDie() < 9) then
        ...
        if S.WorldveinResonance:IsCastableP() and not ShouldStop and (Player:BuffStackP(S.LifebloodBuff) < 4 and Player:BuffDownP(S.TrueshotBuff)) then
            if HR.Cast(S.WorldveinResonance) then return "worldvein_resonance"; end
        end
        ...
        if S.GuardianofAzeroth:IsCastableP() and not ShouldStop and (((Target:HealthPercentage() < 20 or Target:HealthPercentage() > 80) or Target:TimeToDie() > S.GuardianofAzeroth:Cooldown() + S.GuardianofAzeroth:BaseDuration()) and (Player:BuffP(S.TrueshotBuff) or S.Trueshot:CooldownRemainsP() < 16) or Target:TimeToDie() < 31) then
            if HR.Cast(S.GuardianofAzeroth) then return "guardian_of_azeroth"; end
        end
    ...
    local function St()
        -- explosive_shot
        if S.ExplosiveShot:IsCastableP() and not ShouldStop then
            if HR.Cast(S.ExplosiveShot) then return "explosive_shot 126"; end
        end
        -- barrage,if=active_enemies>1
        if S.Barrage:IsReadyP() and not ShouldStop and (EnemiesCount > 1) then
        ...
        if S.SerpentSting:IsCastableP() and not ShouldStop and (Target:DebuffRefreshableCP(S.SerpentStingDebuff) and not S.SerpentSting:InFlight()) then
            if HR.Cast(S.SerpentSting) then return "serpent_sting 138"; end
        end
        -- rapid_fire,if=buff.trueshot.down|focus<70
        if S.RapidFire:IsCastableP() and not ShouldStop and (Player:BuffDownP(S.TrueshotBuff) or Player:Focus() < 70) then
        ...
        if S.BloodoftheEnemy:IsCastableP() and not ShouldStop and (Player:BuffP(S.TrueshotBuff) and (Player:BuffStackP(S.UnerringVisionBuff) > 4 or not S.UnerringVision:AzeriteEnabled()) or Target:TimeToDie() < 11) then
        ...
        if S.FocusedAzeriteBeam:IsCastableP() and not ShouldStop and (Player:BuffDownP(S.TrueshotBuff) or Target:TimeToDie() < 5) then
        ...
        if S.ArcaneShot:IsCastableP() and not ShouldStop and (Player:BuffP(S.TrueshotBuff) and MasterMarksmanBuffCheck() and not Player:BuffP(S.MemoryofLucidDreams)) then
        ...
        -- arcane_shot,if=buff.trueshot.up&buff.master_marksman.up&buff.memory_of_lucid_dreams.up
        if S.ArcaneShot:IsCastableP() and not ShouldStop and (Player:BuffP(S.TrueshotBuff) and MasterMarksmanBuffCheck() and Player:BuffP(S.MemoryofLucidDreams)) then
            if HR.Cast(S.ArcaneShot) then return "arcane_shot 176"; end
        end
        -- piercing_shot
        if S.PiercingShot:IsCastableP() and not ShouldStop then
            if HR.Cast(S.PiercingShot) then return "piercing_shot 198"; end
        end
        -- purifying_blast,if=!buff.trueshot.up|target.time_to_die<8
        if S.PurifyingBlast:IsCastableP() and not ShouldStop and (Player:BuffDownP(S.TrueshotBuff) or Target:TimeToDie() < 8) then
        ...
        if S.ArcaneShot:IsCastableP() and not ShouldStop and (Player:BuffDownP(S.TrueshotBuff) and (Player:BuffP(S.PreciseShotsBuff) and (Player:Focus() > 41 or MasterMarksmanBuffCheck()) or (Player:Focus() > 50 and S.FocusedFire:IsAvailable() or Player:Focus() > 75) and (S.Trueshot:CooldownRemainsP() > 5 or Player:Focus() > 80) or Target:TimeToDie() < 5)) then
            if HR.Cast(S.ArcaneShot) then return "arcane_shot 200"; end
        end
        -- steady_shot
        if S.SteadyShot:IsCastableP() and not ShouldStop then
    ...
    local function Trickshots()
        ...
        if S.RapidFire:IsCastableP() and not ShouldStop and (Player:BuffP(S.TrickShotsBuff) and (S.FocusedFire:AzeriteEnabled() or S.IntheRhythm:AzeriteRank() > 1 or S.SurgingShots:AzeriteEnabled() or S.Streamline:IsAvailable())) then
        ...
        if S.RapidFire:IsCastableP() and not ShouldStop and (Player:BuffP(S.TrickShotsBuff)) then
        ...
        if S.Multishot:IsCastableP() and not ShouldStop and (Player:BuffDownP(S.TrickShotsBuff) or Player:BuffP(S.PreciseShotsBuff) and Player:BuffDownP(S.TrueshotBuff) or Player:Focus() > 70) then
