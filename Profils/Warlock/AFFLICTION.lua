...
local function APL() 
    ...
    local function Cooldowns()
        ...
        if S.Fireblood:IsCastableP() and not ShouldStop and HR.CDsON() and (not S.SummonDarkglare:CooldownUpP()) then
        ...
        if S.BloodFury:IsCastableP() and not ShouldStop and HR.CDsON() and (not S.SummonDarkglare:CooldownUpP()) then
            if HR.Cast(S.BloodFury, Action.GetToggle(2, "OffGCDasOffGCD")) then return "blood_fury 55"; end
        end
        -- memory_of_lucid_dreams,if=time>30
        if S.MemoryofLucidDreams:IsCastableP() and not ShouldStop and HR.CDsON() and (HL.CombatTime() > 30) then
        ...
        if I.PocketsizedComputationDevice:IsEquipped() and TrinketON() and I.PocketsizedComputationDevice:IsReady() and Action.AbsentImun(nil, unit, "DamageMagicImun") and (S.SummonDarkglare:CooldownRemainsP() >= 25 and (bool(S.Deathbolt:CooldownRemainsP()) or not S.Deathbolt:IsAvailable())) then
        ...
        if I.RotcrustedVoodooDoll:IsEquipped() and TrinketON() and not ShouldStop and I.RotcrustedVoodooDoll:IsReady() and (S.SummonDarkglare:CooldownRemainsP() >= 25 and (bool(S.Deathbolt:CooldownRemainsP()) or not S.Deathbolt:IsAvailable())) then
        ...
        if I.AquipotentNautilus:IsEquipped() and TrinketON() and not ShouldStop and I.AquipotentNautilus:IsReady() and (S.SummonDarkglare:CooldownRemainsP() >= 25 and (bool(S.Deathbolt:CooldownRemainsP()) or not S.Deathbolt:IsAvailable())) then
        ...
        if I.TidestormCodex:IsEquipped() and TrinketON() and not ShouldStop and I.TidestormCodex:IsReady() and (S.SummonDarkglare:CooldownRemainsP() >= 25 and (bool(S.Deathbolt:CooldownRemainsP()) or not S.Deathbolt:IsAvailable())) then
        ...
        if I.VialofStorms:IsEquipped() and TrinketON() and not ShouldStop and I.VialofStorms:IsReady() and (S.SummonDarkglare:CooldownRemainsP() >= 25 and (bool(S.Deathbolt:CooldownRemainsP()) or not S.Deathbolt:IsAvailable())) then
    ...
    local function Fillers()
        ...
        if S.Deathbolt:IsCastableP() and not ShouldStop and ActiveUAs() >= 1 and Player:SoulShardsP() <= 1 and (S.SummonDarkglare:CooldownRemainsP() >= 30 + Player:GCD() or S.SummonDarkglare:CooldownRemainsP() > 140) then
        ...
        if S.FocusedAzeriteBeam:IsCastableP() and not ShouldStop and Action.GetToggle(1, "HeartOfAzeroth") and not ShouldStop then
            if HR.Cast(S.UnleashHeartOfAzeroth) then return "focused_azerite_beam 463"; end
        end
        -- purifying_blast
        if S.PurifyingBlast:IsCastableP() and not ShouldStop and Action.GetToggle(1, "HeartOfAzeroth") and not ShouldStop then
    ...
    local function Spenders()
        ...
        if S.UnstableAffliction:IsReadyP() and not ShouldStop and S.CascadingCalamity:AzeriteEnabled() and not Player:BuffP(S.CascadingCalamityBuff) and ActiveUAs() >= 1 and contagion <= S.UnstableAffliction:CastTime() + VarPadding + 1 then
    ...
    if Player:AffectingCombat() and not PrepareAshvaneBurst() then
        ...
        if S.Deathbolt:IsCastableP() and not ShouldStop and ActiveUAs() >= 1 and Player:SoulShardsP() <= 1 and (S.SummonDarkglare:CooldownRemainsP() >= 30 and EnemiesCount == 1 and (not S.VisionofPerfectionMinor:IsAvailable() and not bool(S.DreadfulCalling:AzeriteRank()) or S.SummonDarkglare:CooldownRemainsP() > 30)) then
        ...
        if S.PhantomSingularity:IsCastableP() and (S.SummonDarkglare:CooldownRemainsP() >= 45 or S.SummonDarkglare:IsReady()) and not ShouldStop and EvaluateTargetIfFilterPhantomSingularity841(Target) and EvaluateTargetIfPhantomSingularity850(Target) then
        ...
        if S.UnstableAffliction:IsReadyP() and not ShouldStop and EvaluateTargetIfFilterUnstableAffliction865(Target) and EvaluateTargetIfUnstableAffliction870(Target) then
        ...
        if I.AzsharasFontofPower:IsEquipped() and TrinketON() and not ShouldStop and I.AzsharasFontofPower:IsReady() and (HL.CombatTime() <= 3) then
            if HR.Cast(I.AzsharasFontofPower) then return "azsharas_font_of_power 879"; end
        end
        -- phantom_singularity,if=time<=35
        if S.PhantomSingularity:IsCastableP() and not ShouldStop and (HL.CombatTime() <= 35) then
        ...
        if S.DarkSoulMisery:IsCastableP() and not ShouldStop and HR.CDsON() and (S.SummonDarkglare:CooldownRemainsP() < 15 + Player:SoulShardsP() * num(S.DreadfulCalling:AzeriteEnabled()) and (Target:DebuffP(S.PhantomSingularityDebuff) or Target:DebuffP(S.PhantomSingularityDebuff) or not S.PhantomSingularity:IsAvailable() and not S.VileTaint:IsAvailable()) or Target:TimeToDie() < 20 + Player:GCD() or EnemiesCount > 1) then
        ...
        -- berserking
        if S.Berserking:IsReadyP() and not ShouldStop and HR.CDsON() then
