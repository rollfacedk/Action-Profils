...
local function APL() 
    ...
    if Everyone.TargetIsValid() then
        ...
        -- demonbolt
        if S.Demonbolt:IsCastableP() and not ShouldStop then
  ...
  local function Opener()
    ...
    if S.HandofGuldan:IsReadyP() and not ShouldStop and HL.CombatTime() < 2 and not Player:PrevGCDP(1, S.HandofGuldan) and Player:SoulShardsP() > 2 and (S.ExplosivePotential:AzeriteEnabled()) then
    ...
    if S.Implosion:IsReadyP() and not ShouldStop and (S.ExplosivePotential:AzeriteEnabled() and WildImpsCount() > 2 and Player:BuffDownP(S.ExplosivePotentialBuff)) then
        if HR.Cast(S.Implosion) then return "implosion 31"; end
    end
    -- doom,line_cd=30
    -- Manually added DebuffDownP check to avoid getting stuck at this line
    if S.Doom:IsCastableP() and not ShouldStop and (Target:DebuffDownP(S.DoomDebuff)) then
        if HR.Cast(S.Doom) then return "doom 32"; end
    end
    -- guardian_of_azeroth
    if S.GuardianofAzeroth:IsCastableP() and not ShouldStop and Action.GetToggle(1, "HeartOfAzeroth") and HR.CDsON() then
    ...
    if S.HandofGuldan:IsReadyP() and not ShouldStop and (Player:PrevGCDP(1, S.HandofGuldan) and Player:SoulShardsP() > 0 and Player:PrevGCDP(2, S.SoulStrike)) then
    ...
    if S.DemonicStrength:IsCastableP() and not ShouldStop and (Player:PrevGCDP(1, S.HandofGuldan) and not Player:PrevGCDP(2, S.HandofGuldan) and (WildImpsCount() > 1 and S.HandofGuldan:InFlight())) then
        if HR.Cast(S.DemonicStrength, Action.GetToggle(2, "OffGCDasOffGCD")) then return "demonic_strength 35"; end
    end
    -- bilescourge_bombers
    if S.BilescourgeBombers:IsReadyP() and not ShouldStop then
    ...
    if S.SoulStrike:IsCastableP() and not ShouldStop and (not Player:HasHeroism() or HL.CombatTime() > 5 and Player:PrevGCDP(1, S.HandofGuldan)) then
        if HR.Cast(S.SoulStrike) then return "soul_strike 37"; end
    end
    -- summon_vilefiend,if=soul_shard=5
    if S.SummonVilefiend:IsReadyP() and not ShouldStop and (Player:SoulShardsP() == 5) then
        if HR.Cast(S.SummonVilefiend) then return "summon_vilefiend 38"; end
    end
    -- grimoire_felguard,if=soul_shard=5
    if S.GrimoireFelguard:IsReadyP() and not ShouldStop and (Player:SoulShardsP() == 5) then
    ...
    if S.CallDreadstalkers:IsReadyP() and not ShouldStop and (Player:SoulShardsP() == 5) then
        if HR.Cast(S.CallDreadstalkers) then return "call_dreadstalkers 40"; end
    end
    -- hand_of_guldan,if=soul_shard=5
    if S.HandofGuldan:IsReadyP() and not ShouldStop and (Player:SoulShardsP() == 5) then
    ...
    if S.HandofGuldan:IsReadyP() and not ShouldStop and (Player:SoulShardsP() >= 3 and Player:PrevGCDP(2, S.HandofGuldan) and HL.CombatTime() > 5 and (Player:PrevGCDP(1, S.SoulStrike) or not S.SoulStrike:IsAvailable() and Player:PrevGCDP(1, S.ShadowBolt))) then
    ...
    if S.SummonDemonicTyrant:IsReadyP() and not ShouldStop and HR.CDsON() and (Player:PrevGCDP(1, S.DemonicStrength) or Player:PrevGCDP(1, S.HandofGuldan) and Player:PrevGCDP(2, S.HandofGuldan) or not S.DemonicStrength:IsAvailable() and WildImpsCount() + ImpsSpawnedDuring(S.SummonDemonicTyrant:CastTime()) >= 6) then
  ...
  local function Implosion()
    ...
    if S.GrimoireFelguard:IsReadyP() and not ShouldStop and (S.SummonDemonicTyrant:CooldownRemainsP() < 13) then
    ...
    if S.HandofGuldan:IsCastableP() and not ShouldStop and (Player:SoulShardsP() >= 3 and (((Player:PrevGCDP(2, S.HandofGuldan) or WildImpsCount() >= 3) and WildImpsCount() < 9) or S.SummonDemonicTyrant:CooldownRemainsP() <= Player:GCD() * 2 or Player:BuffRemainsP(S.DemonicPowerBuff) > Player:GCD() * 2)) then
    ...
    if S.Demonbolt:IsCastableP() and not ShouldStop and (Player:PrevGCDP(1, S.HandofGuldan) and Player:SoulShardsP() >= 1 and (WildImpsCount() <= 3 or Player:PrevGCDP(3, S.HandofGuldan)) and Player:SoulShardsP() < 4 and Player:BuffP(S.DemonicCoreBuff)) then
    ...
    if S.SummonVilefiend:IsReadyP() and not ShouldStop and ((S.SummonDemonicTyrant:CooldownRemainsP() > 40 and EnemiesCount <= 2) or S.SummonDemonicTyrant:CooldownRemainsP() < 12) then
    ...
    if S.BilescourgeBombers:IsReadyP() and not ShouldStop and (S.SummonDemonicTyrant:CooldownRemainsP() > 9) then
    ...
    if S.ConcentratedFlame:IsCastableP() and not ShouldStop and Action.GetToggle(1, "HeartOfAzeroth") and (Target:DebuffDownP(S.ConcentratedFlameBurn) and not S.ConcentratedFlame:InFlight() and EnemiesCount < 5) then
    ...
    if S.SoulStrike:IsCastableP() and not ShouldStop and (Player:SoulShardsP() < 5 and Player:BuffStackP(S.DemonicCoreBuff) <= 2) then
    ...
    if S.Demonbolt:IsCastableP() and not ShouldStop and (Player:SoulShardsP() <= 3 and Player:BuffP(S.DemonicCoreBuff) and (Player:BuffStackP(S.DemonicCoreBuff) >= 3 or Player:BuffRemainsP(S.DemonicCoreBuff) <= Player:GCD() * 5.7)) then
  ...
  local function NetherPortalActive()
    ...
    if S.SummonVilefiend:IsReadyP() and not ShouldStop and (S.SummonDemonicTyrant:CooldownRemainsP() > 40 or S.SummonDemonicTyrant:CooldownRemainsP() < 12) then
    ...
    if S.HandofGuldan:IsCastableP() and not ShouldStop and (Player:SoulShardsP() > 0 and ((S.CallDreadstalkers:CooldownRemainsP() > S.Demonbolt:CastTime()) and (S.CallDreadstalkers:CooldownRemainsP() > S.ShadowBolt:CastTime())) and S.NetherPortal:CooldownRemainsP() > (165 + S.HandofGuldan:CastTime())) then
    ...
    if S.SummonDemonicTyrant:IsCastableP() and not ShouldStop and (Player:BuffRemainsP(S.NetherPortalBuff) < 5 and Player:SoulShardsP() == 0) then
    ...
    if S.SummonDemonicTyrant:IsCastableP() and not ShouldStop and (Player:BuffRemainsP(S.NetherPortalBuff) < S.SummonDemonicTyrant:CastTime() + 0.5) then
    ...
    if S.Demonbolt:IsCastableP() and not ShouldStop and (Player:BuffP(S.DemonicCoreBuff) and Player:SoulShardsP() <= 3) then
  ...
  local function NetherPortalBuilding()
    ...
    if S.GuardianofAzeroth:IsCastableP() and not ShouldStop and Action.GetToggle(1, "HeartOfAzeroth") and (S.NetherPortal:CooldownUpP() and Player:SoulShardsP() >= 5) then
        if HR.Cast(S.GuardianofAzeroth) then return "guardian_of_azeroth 296"; end
    end
    -- nether_portal,if=soul_shard>=5
    if S.NetherPortal:IsReadyP() and not ShouldStop and (Player:SoulShardsP() >= 5) then
    ...
    if S.CallDreadstalkers:IsReadyP() and not ShouldStop and (HL.CombatTime() >= 30) then
    ...
    if S.HandofGuldan:IsCastableP() and not ShouldStop and (HL.CombatTime() >= 30 and S.CallDreadstalkers:CooldownRemainsP() > 18 and Player:SoulShardsP() >= 3) then
    ...
    if S.PowerSiphon:IsCastableP() and not ShouldStop and (HL.CombatTime() >= 30 and WildImpsCount() >= 2 and Player:BuffStackP(S.DemonicCoreBuff) <= 2 and Player:BuffDownP(S.DemonicPowerBuff) and Player:SoulShardsP() >= 3) then
        if HR.Cast(S.PowerSiphon) then return "power_siphon 309"; end
    end
    -- hand_of_guldan,if=time>=30&soul_shard>=5
    if S.HandofGuldan:IsCastableP() and not ShouldStop and (HL.CombatTime() >= 30 and Player:SoulShardsP() >= 5) then
   ...
   -- end
    ...
    if S.Berserking:IsCastableP() and not ShouldStop and HR.CDsON() and (DemonicTyrantTime() > 0 and (not S.VisionofPerfection:IsAvailable() or not S.DemonicConsumption:IsAvailable() or S.SummonDemonicTyrant:CooldownRemainsP() >= S.SummonDemonicTyrant:BaseDuration() - 5) or Target:TimeToDie() <= 15) then
    ...
    if S.BloodFury:IsCastableP() and not ShouldStop and HR.CDsON() and (DemonicTyrantTime() > 0 and (not S.VisionofPerfection:IsAvailable() or not S.DemonicConsumption:IsAvailable() or S.SummonDemonicTyrant:CooldownRemainsP() >= S.SummonDemonicTyrant:BaseDuration() - 5) or Target:TimeToDie() <= 15) then
    ...
    if S.Fireblood:IsCastableP() and not ShouldStop and HR.CDsON() and (DemonicTyrantTime() > 0 and (not S.VisionofPerfection:IsAvailable() or not S.DemonicConsumption:IsAvailable() or S.SummonDemonicTyrant:CooldownRemainsP() >= S.SummonDemonicTyrant:BaseDuration() - 5) or Target:TimeToDie() <= 15) then
    ...
    if S.BloodoftheEnemy:IsCastableP() and not ShouldStop and Action.GetToggle(1, "HeartOfAzeroth") and (DemonicTyrantTime() > 0 and DemonicTyrantTime() <= 15 - Player:GCD() * 3 and (not S.VisionofPerfection:IsAvailable() or not S.DemonicConsumption:IsAvailable() or S.SummonDemonicTyrant:CooldownRemainsP() >= S.SummonDemonicTyrant:BaseDuration() - 5)) then
    ...
    if S.RippleInSpace:IsCastableP() and not ShouldStop and Action.GetToggle(1, "HeartOfAzeroth") and (DemonicTyrantTime() > 0 and (not S.VisionofPerfection:IsAvailable() or not S.DemonicConsumption:IsAvailable() or S.SummonDemonicTyrant:CooldownRemainsP() >= S.SummonDemonicTyrant:BaseDuration() - 5) or Target:TimeToDie() <= 15) then
    ...
    if S.HandofGuldan:IsCastableP() and not ShouldStop and (bool(S.ExplosivePotential:AzeriteRank()) and HL.CombatTime() < 5 and Player:SoulShardsP() > 2 and Player:BuffDownP(S.ExplosivePotentialBuff) and WildImpsCount() < 3 and not Player:PrevGCDP(1, S.HandofGuldan) and not Player:PrevGCDP(2, S.HandofGuldan)) then
    ...
    if S.Demonbolt:IsCastableP() and not ShouldStop and (Player:SoulShardsP() <= 3 and Player:BuffP(S.DemonicCoreBuff) and Player:BuffStackP(S.DemonicCoreBuff) == 4) then
    ...
    if S.Implosion:IsCastableP() and not ShouldStop and (bool(S.ExplosivePotential:AzeriteRank()) and WildImpsCount() > 2 and Player:BuffRemainsP(S.ExplosivePotentialBuff) < S.ShadowBolt:ExecuteTime() and (not S.DemonicConsumption:IsAvailable() or S.SummonDemonicTyrant:CooldownRemainsP() > 12)) then
    ...
    if S.Doom:IsCastableP() and not ShouldStop and (not Target:DebuffP(S.DoomDebuff) and Target:TimeToDie() > 30 and EnemiesCount < 2 and Player:BuffDownP(S.NetherPortalBuff)) then
    ...
    if S.BilescourgeBombers:IsReadyP() and not ShouldStop and (S.ExplosivePotential:AzeriteRank() > 0 and HL.CombatTime() < 10 and EnemiesCount < 2 and DreadStalkersTime() > 0 and S.NetherPortal:IsAvailable()) then
    ...
    if S.DemonicStrength:IsReadyP() and not ShouldStop and ((WildImpsCount() < 6 or Player:BuffP(S.DemonicPowerBuff)) or EnemiesCount < 2) then
    ...
    if S.GrimoireFelguard:IsReadyP() and not ShouldStop and ((Target:TimeToDie() > 120 or Target:TimeToDie() < S.SummonDemonicTyrant:CooldownRemainsP() + 15 or S.SummonDemonicTyrant:CooldownRemainsP() < 13)) then
    ...
    if S.HandofGuldan:IsCastableP() and not ShouldStop and ((S.BalefulInvocation:AzeriteEnabled() or S.DemonicConsumption:IsAvailable()) and Player:PrevGCDP(1, S.HandofGuldan) and S.SummonDemonicTyrant:CooldownRemainsP() < 2 and Player:SoulShardsP() > 0) then
    ...
    if S.PowerSiphon:IsCastableP() and not ShouldStop and (WildImpsCount() >= 2 and Player:BuffStackP(S.DemonicCoreBuff) <= 2 and Player:BuffDownP(S.DemonicPowerBuff) and EnemiesCount < 2) then
    ...
    if S.Doom:IsCastableP() and not ShouldStop and (S.Doom:IsAvailable() and Target:DebuffRefreshableCP(S.DoomDebuff) and Target:TimeToDie() > (Target:DebuffRemainsP(S.DoomDebuff) + 30)) then
    ...
    if S.ConcentratedFlame:IsCastableP() and not ShouldStop and Action.GetToggle(1, "HeartOfAzeroth") and (Target:DebuffDownP(S.ConcentratedFlameBurn) and not S.ConcentratedFlame:InFlight() and DemonicTyrantTime() == 0) then
