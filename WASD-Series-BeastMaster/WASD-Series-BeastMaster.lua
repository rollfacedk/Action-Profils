local function MyRoutine()
	local Author = 'Rollface 11.2|v1.0'
	...
	local S = {
	  ...
	  PoolFocus                             = Spell(999910),
		...
		FortitudeoftheBear = Spell(272679),
		FortitudeoftheBearBuff = Spell(272679),
		SurvivaloftheFittest = Spell(264735),
		Camouflage = Spell(199483),
	...
	local function Init()
		...
		message( 'Rollface 11.2|v1.0', 1)
		MainAddon:Print('Rollface 11.2|v1.0')
	end
	...
	local function DRCleave()
		-- kill_shot
		if S.BlackArrow:IsReady() and Target:IsSpellInRange(S.BlackArrow) then
		...
		if S.BarbedShot:IsCastable() and Target:IsSpellInRange(S.BarbedShot) and (S.BarbedShot:FullRechargeTime() < Player:GCD()) then
		  if CastTargetIf(S.BarbedShot, Enemies40y, "min", EvaluateTargetIfFilterBarbedShot) then return "barbed_shot dr_cleave 6"; end
		end
		-- bloodshed
		...
		if S.MultiShot:IsReady() and not Target:IsSpellInRange(S.MultiShot) and (Pet:BuffDown(S.BeastCleavePetBuff) and (not S.BloodyFrenzy:IsAvailable() or S.CalloftheWild:CooldownDown())) then
		...
		if S.ExplosiveShot:IsReady() and TargetInRange40y and (S.ThunderingHooves:IsAvailable()) then
		...
		if S.BarbedShot:IsCastable() and Target:IsSpellInRange(S.BarbedShot) and (S.BarbedShot:ChargesFractional() >= S.KillCommand:ChargesFractional()) then
		  if CastTargetIf(S.BarbedShot, Enemies40y, "min", EvaluateTargetIfFilterBarbedShot) then return "barbed_shot dr_cleave 16"; end
		end
		-- kill_command
		if S.KillCommand:IsReady() and Target:IsSpellInRange(S.KillCommand) then
		  if Cast(S.KillCommand) then return "kill_command dr_cleave 18"; end
		end
		-- cobra_shot,if=focus.time_to_max<gcd*2
		if S.CobraShot:IsReady() and Target:IsSpellInRange(S.CobraShot) and (Player:FocusTimeToMax() < Player:GCD() * 2) then
	...
	local function DRST()
		...
		if S.BarbedShot:IsCastable() and Target:IsSpellInRange(S.BarbedShot) and (Player:BuffRemains(S.ThrilloftheHuntBuff) < Player:GCD() * 1.5 or not S.ThrilloftheHunt:IsAvailable()) then
		...
		if S.BarbedShot:IsCastable() and Target:IsSpellInRange(S.BarbedShot) and (WFTTRCheck and (S.BarbedShot:FullRechargeTime() < Player:GCD() or S.BestialWrath:CooldownRemains() < 12 and Player:BuffUp(S.WitheringFireBuff))) then
		...
		if S.KillCommand:IsReady() and Target:IsSpellInRange(S.KillCommand) and (Player:BuffDown(S.WitheringFireBuff) or WFTTR > Player:GCD() and S.BlackArrow:CooldownRemains() > 0.5) then
		...
		-- Note: ST function, so only using Cast instead of CastTargetIf.
		if S.BarbedShot:IsCastable() and Target:IsSpellInRange(S.BarbedShot) and (WFTTRCheck) then
		...
		if S.CobraShot:IsReady() and Target:IsSpellInRange(S.CobraShot) and (WFTTRCheck) then
	...
	local function Cleave()
		...
		if S.BestialWrath:IsCastable() and (Player:BuffRemains(S.HowlofthePackLeaderCDBuff) - 12 < 12 / Player:GCD() * 0.5 or not Player:HasTier("TWW3", 4) or S.MultiShot:IsAvailable()) then
		...
		if S.BarbedShot:IsCastable() and Target:IsSpellInRange(S.BarbedShot) and (S.BarbedShot:FullRechargeTime() < Player:GCD() or S.BarbedShot:ChargesFractional() >= S.KillCommand:ChargesFractional() or S.CalloftheWild:IsAvailable() and S.CalloftheWild:CooldownUp() or HowlSummonReady() and S.BarbedShot:FullRechargeTime() < 8) then
		  if CastTargetIf(S.BarbedShot, Enemies40y, "min", EvaluateTargetIfFilterBarbedShot) then return "barbed_shot cleave 4"; end
		end
		-- bloodshed
		if S.Bloodshed:IsCastable() and Target:IsSpellInRange(S.Bloodshed) then
		  if Cast(S.Bloodshed) then return "bloodshed cleave 6"; end
		end
		-- multishot,if=pet.main.buff.beast_cleave.down&(!talent.bloody_frenzy|cooldown.call_of_the_wild.remains)
		if S.MultiShot:IsReady() and Target:IsSpellInRange(S.MultiShot) and (Pet:BuffDown(S.BeastCleavePetBuff) and (not S.BloodyFrenzy:IsAvailable() or S.CalloftheWild:CooldownDown() or not CDsON())) then
		...
		-- kill_command
		if S.KillCommand:IsReady() and Target:IsSpellInRange(S.KillCommand) then
		  if Cast(S.KillCommand) then return "kill_command cleave 14"; end
		end
		-- cobra_shot,if=focus.time_to_max<gcd*2|buff.hogstrider.stack>3|!talent.multishot
		if S.CobraShot:IsReady() and Target:IsSpellInRange(S.CobraShot) and (Player:FocusTimeToMax() < Player:GCD() * 2 or Player:BuffStack(S.HogstriderBuff) > 3 or not S.MultiShot:IsAvailable()) then
	...
	local function ST()
		...
		if S.BestialWrath:IsCastable() and (Player:BuffRemains(S.HowlofthePackLeaderCDBuff) - 12 < 12 / Player:GCD() * 0.5 or not Player:HasTier("TWW3", 4)) then
		  if Cast(S.BestialWrath) then return "bestial_wrath st 2"; end
		end
		-- barbed_shot,target_if=min:dot.barbed_shot.remains,if=full_recharge_time<gcd
		...
		-- Main Target backup
		if S.BarbedShot:IsCastable() and Target:IsSpellInRange(S.BarbedShot) and (S.BarbedShot:FullRechargeTime() < Player:GCD()) then
		  if Cast(S.BarbedShot) then return "barbed_shot st mt_backup 6"; end
		end
		-- call_of_the_wild
		if S.CalloftheWild:IsCastable() then
		  if Cast(S.CalloftheWild) then return "call_of_the_wild st 8"; end
		end
		-- bloodshed
		...
		if S.KillCommand:IsReady() and Target:IsSpellInRange(S.KillCommand) and (S.KillCommand:ChargesFractional() >= S.BarbedShot:ChargesFractional() and not (Player:BuffRemains(S.LeadFromTheFrontBuff) > Player:GCD() and Player:BuffRemains(S.LeadFromTheFrontBuff) < Player:GCD() * 2 + 0.5 and not HowlSummonReady() and S.KillCommand:FullRechargeTime() > Player:GCD())) then
		  if Cast(S.KillCommand) then return "kill_command st 12"; end
		end
		-- barbed_shot,target_if=min:dot.barbed_shot.remains
		if S.BarbedShot:IsCastable() and Target:IsSpellInRange(S.KillCommand) then
		  if CastTargetIf(S.BarbedShot, Enemies40y, "min", EvaluateTargetIfFilterBarbedShot) then return "barbed_shot cleave 14"; end
		end
		-- cobra_shot
		if S.CobraShot:IsReady() and Target:IsSpellInRange(S.CobraShot) then
		  if Cast(S.CobraShot) then return "cobra_shot st 16"; end
		end
	end
	...
	local function MainRotation()
		...
		if 	Player:AffectingCombat() and
			...
			Player:BuffDown(S.FortitudeoftheBearBuff) and 
			...
			elseif S.FortitudeoftheBear:IsCastable() then
				if Cast(S.FortitudeoftheBear) then return end
