...
Action[ACTION_CONST_WARLOCK_AFFLI] = {
    ...
    BurningRush                          = Action.Create({ Type = "Spell", ID = 278727, Hidden = true     }),
...
local function APL() 
	...
	local function Cds()
        ...
        if I.AzsharasFontofPower:IsEquipped() and I.AzsharasFontofPower:IsReady() and Settings.Commons.UseTrinkets and (S.SummonInfernal:CooldownUpP() or S.SummonInfernal:CooldownRemainsP() < 5) then
            if HR.Cast(I.AzsharasFontofPower) then return "azsharas_font_of_power 159"; end
        end
