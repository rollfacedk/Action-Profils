...
A.Data.ProfileUI = {    
            ...
            { -- [1]                            
                {
                    ...
                    L = { 
						...
						frFR = "Utiliser @mouseover",
                    }, 
                    TT = { 
                        enUS = "Will unlock use actions for @mouseover units\nExample: Resuscitate, Healing", 
                        ruRU = "Разблокирует использование действий для @mouseover юнитов\nНапример: Воскрешение, Хилинг", 
						frFR = "Autorisera des actions pour les unités en @mouseover\nExemple: Resusciter, Heal",
                    }, 
                    M = {},
                },
                {
                    ...
                    TT = { 
						...
						frFR = "Active les actions pour les unités multiples"
                    }, 
                    M = {},
                },                    
            }, 
    ...
    [7] = {
        [ACTION_CONST_WARLOCK_AFFLI] = { 
		    -- 
            ["kick"] = { Enabled = true, Key = "SpellLock", LUA = [[
                return     Unit("player"):HasBuffs(Action[PlayerSpec].ZenMeditation.ID, true) == 0 and
                        SpellInRange(thisunit, Action[PlayerSpec].SpellLock.ID) and 
                        select(2, CastTime(nil, thisunit)) > 0 and 
                        Action[PlayerSpec].SpellLock:AbsentImun(thisunit, {"KickImun", "TotalImun", "DamagePhysImun"}, true) 
            ]] },
			-- DispellMagic Imp Pet
            ["dispel"] = { Enabled = true, Key = "Detox", LUA = [[
                return  SpellInRange(thisunit, Action[PlayerSpec].DispellMagic.ID) and 
                        Action.AuraIsValid(thisunit, "UseDispel", "Dispel") and                         
                        Action[PlayerSpec].DispellMagic:AbsentImun(thisunit) and 
        ...
        [ACTION_CONST_WARLOCK_DEMO] = {
            ["petstun"] = { Enabled = true, Key = "PetStun", LUA = [[
                return     (
                            not InPvP() and 
                            AoE(1, 5 + (TalentLearn(Action[PlayerSpec].TigerTailSweep.ID) and 2 or 0)) 
                        ) or 
...
--[[function Env.GrappleWeaponIsReady(unit, isMsg)
    if A[Env.PlayerSpec].GrappleWeapon then 
        ...
        A[Env.PlayerSpec].GrappleWeapon:AbsentImun(unit, {"CCTotalImun", "DamageMagicImun", "TotalImun"}, true) and 
        Env.Unit(unit):IsControlAble("disarm", 0) and 
        Env.Unit(unit):HasDeBuffs("Disarmed") == 0
    end 
end ]]--

-- SpellLock PvP Interrupt W/O Toggle
function Env.Main_CastBars(unit, list)
    if not A.IsInitialized or Env.IamHealer or not Env.InPvP() then 
        return false 
    end 
    
    if A[Env.PlayerSpec] and A[Env.PlayerSpec].SpellLock and A[Env.PlayerSpec].SpellLock:IsReadyP(unit, nil, true) and A[Env.PlayerSpec].SpellLock:AbsentImun(unit, {"KickImun", "TotalImun", "DamageMagicImun"}, true) and A.InterruptIsValid(unit, list) then 
        return true         
    end 
end 

-- SpellLock PvP Interrupt Toggle
function Env.Second_CastBars(unit)
    if not A.IsInitialized or not Env.InPvP() then 
        return false 
    end 
    
    local Toggle = A.GetToggle(2, "SpellLockPvP")    
    if Toggle and Toggle ~= "OFF" and A[Env.PlayerSpec] and A[Env.PlayerSpec].SpellLock and A[Env.PlayerSpec].SpellLock:IsReadyP(unit, nil, true) and A[Env.PlayerSpec].SpellLock:AbsentImun(unit, {"KickImun", "TotalImun", "DamageMagicImun"}, true) then 
        if Toggle == "BOTH" then 
