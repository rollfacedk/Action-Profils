local function MyRoutine()
	...
	local function HolyPrismFunc(UnitTarget)
		return UnitTarget:HealthPercentage() <= 45 or UnitTarget:DebuffUp(S.EnvelopingShadowflame) or UnitTarget:DebuffUp(S.VoidRift) or UnitTarget:DebuffUp(S.CurseOfEntropy) or UnitTarget:DebuffUp(S.CorruptedCoating)
	end;
