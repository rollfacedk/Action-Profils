local function MyRoutine()
	...
	local function MainRotation()
		...
		if S.BeaconOfVirtue:IsCastable() and (HealingEngine:MembersUnderPercentage(75) >= 4 or HealingEngine:DebuffTotal(S.EnvelopingShadowflame) >= 3 or HealingEngine:DebuffTotal(S.CurseOfEntropy) >= 3) then		
		...
		if S.HolyPrism:IsReady() and TargetIsValid() and (HealingEngine:MembersUnderPercentage(75) >= 4 and Player:HolyPower() <= 4 or HealingEngine:DebuffTotal(S.EnvelopingShadowflame) >= 3 or HealingEngine:DebuffTotal(S.CurseOfEntropy) >= 3) then
			if MainAddon.SetTopColor(6, "Holy Prism Enemy") then return end
		end

		if S.DivineToll:IsCastable() and (HealingEngine:MembersUnderPercentage(75) >= 4 and Player:HolyPower() <= 2 or HealingEngine:DebuffTotal(S.EnvelopingShadowflame) >= 3 or HealingEngine:DebuffTotal(S.CurseOfEntropy) >= 3) then
