local function MyRoutine()
	local Author = 'WASD Series - Restoration Shaman'
	...
	local S = {
		...
		SpiritLinkTotemBuff = Spell(325174),
		PrimordialWave = MultiSpell(375982, 428332),
		UnleashLife = Spell(73685),
		HealingRain = MultiSpell(73920, 444995),
	...
	local function HealingWaveFunc(UnitTarget)
		return UnitTarget:DebuffUp(S.EnvelopingShadowflame) or UnitTarget:DebuffUp(S.VoidRift) or UnitTarget:DebuffUp(S.CurseOfEntropy) or UnitTarget:DebuffUp(S.CorruptedCoating)
	end;
	...
	local function ChainHealNaturesSwiftnessFunc(UnitTarget)
		return UnitTarget:HealthPercentage() <= 100 and Player:BuffUp(S.NaturesSwiftness)
	end;
	...
	local function MainRotation()
		...
		if Player:AffectingCombat() and Player:HealthPercentageFlat() <= 45 and
		...
		if (HealingEngine:MembersUnderPercentage(80, nil, 30) >= 4) then
			if S.Wellspring:IsCastable() then
				if Cast(S.Wellspring) then return end
			end
		end
		...
		if (HealingEngine:MembersUnderPercentage(80, nil, 30) >= 3) then
			...
			if S.CloudburstTotem:IsCastable() and Player:AffectingCombat() and S.LivelyTotems:IsAvailable() and not TotemFinder(S.CloudburstTotem) then
				if Cast(S.CloudburstTotem) then return end
			end

			if S.ChainHeal:IsCastable() then
				if S.NaturesSwiftness:IsCastable() then
					if Cast(S.NaturesSwiftness) then return end
				end
				if MainAddon.CastCycleAlly(S.ChainHeal, MEMBERS, ChainHealNaturesSwiftnessFunc) then return end
			end
			
			if S.TotemicRecall:IsCastable() and Player:AffectingCombat() and (S.CloudburstTotem:Charges() == 0 or S.HealingStreamTotem:Charges() == 0)  then
		...
		if not S.SurgingTotem:IsAvailable() and (HealingEngine:MembersUnderPercentage(90, nil, 40) >= 2) then
			if S.HealingStreamTotem:IsCastable() and not TotemFinder(S.HealingStreamTotem) then
				if Cast(S.HealingStreamTotem) then return end
			end
		end
		...
		if S.HealingSurge:IsCastable() then
			if S.NaturesSwiftness:IsCastable() and Player:AffectingCombat() and Player:IsFriend(Focus) and Focus:HealthPercentage() <= 30 then
				if Cast(S.NaturesSwiftness) then return end
			end
		...
		if TargetIsValid() and S.MasteroftheElements:IsAvailable() then
			if S.LavaBurst:IsReady() and Player:BuffRemains(S.MasteroftheElementsBuff) <= 8 then
				if Player:IsCasting(S.LavaBurst) and S.LavaBurst:Charges() == 2 then return end
		...
		if TargetIsValid() and S.MasteroftheElements:IsAvailable() then
			if S.LavaBurst:IsReady() and Player:BuffStack(S.MasteroftheElementsBuff) < 2 then
				if Player:IsCasting(S.LavaBurst) and S.LavaBurst:Charges() == 2 then return end
				if Cast(S.LavaBurst) then return end
			end
		end
		...
		if not Player:AffectingCombat() then
			if S.GhostWolf:IsCastable() and Player:IsMoving() and Player:BuffDown(S.GhostWolf) and not Player:IsCasting() then 
	...
	IsCastable = HL.AddCoreOverride("Spell.IsCastable", 
		...
		if self == S.HealingWave or self == S.HealingSurge or self == S.ChainHeal or self == S.LightningBolt or self == S.ChainLightning or self == S.LavaBurst then
